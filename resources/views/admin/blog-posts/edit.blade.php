...
@section('title', __t('admin.edit_blog_post'))
@section('breadcrumb', __t('admin.panel') . ' / ' . __t('admin.nav_blog_posts') . ' / ' . __t('admin.edit_blog_post'))

@section('content')
<div class="max-w-3xl">
    <div class="bg-white rounded-xl border border-gray-100 p-6">
        <form method="POST" action="{{ route('admin.blog-posts.update', $blogPost) }}" class="space-y-4">
            @csrf @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="text" name="title_translations[{{ $locale }}]"
                               value="{{ old('title_translations.' . $locale, $blogPost->title_translations[$locale] ?? ($locale === 'az' ? $blogPost->title : '')) }}"
                               {{ $locale === 'az' ? 'required' : '' }}
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-start gap-2">
                        ...
                        <textarea name="excerpt_translations[{{ $locale }}]" rows="2" maxlength="500"
                                  class="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-green-500 bg-gray-50 focus:bg-white transition">{{ old('excerpt_translations.' . $locale, $blogPost->excerpt_translations[$locale] ?? ($locale === 'az' ? $blogPost->excerpt : '')) }}</textarea>
                    </div>
                    @endforeach
                </div>
            </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-start gap-2">
                        ...
                        <textarea name="content_translations[{{ $locale }}]" rows="10"
                                  {{ $locale === 'az' ? 'required' : '' }}
                                  class="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-green-500 bg-gray-50 focus:bg-white transition font-mono {{ $errors->has('content_translations.' . $locale) ? 'border-red-300' : '' }}">{{ old('content_translations.' . $locale, $blogPost->content_translations[$locale] ?? ($locale === 'az' ? $blogPost->content : '')) }}</textarea>
                    </div>
                    @endforeach
                </div>
