...
@section('title', __t('admin.edit_locale'))
@section('breadcrumb', __t('admin.panel') . ' / ' . __t('admin.nav_locales') . ' / ' . __t('admin.edit_locale'))

@section('content')
<div class="max-w-xl">
    <div class="bg-white rounded-xl border border-gray-100 p-6">
        <form method="POST" action="{{ route('admin.locales.update', $locale) }}" class="space-y-4">
            @csrf @method('PUT')

            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">{{ __t('admin.col_code') }}</label>
                    <input type="text" name="code" value="{{ old('code', $locale->code) }}" maxlength="5" required
                ...
                <div>
                    ...
                    <input type="text" name="flag" value="{{ old('flag', $locale->flag) }}" maxlength="10"
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $locale->name) }}" required
            ...
            <div>
                ...
                <input type="text" name="native_name" value="{{ old('native_name', $locale->native_name) }}"
                       class="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-green-500 bg-gray-50 focus:bg-white transition"
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <select name="dir"
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-green-500 bg-gray-50 focus:bg-white transition">
                        <option value="ltr" {{ old('dir', $locale->dir) === 'ltr' ? 'selected' : '' }}>LTR</option>
                        <option value="rtl" {{ old('dir', $locale->dir) === 'rtl' ? 'selected' : '' }}>RTL</option>
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">{{ __t('admin.col_sort_order') }}</label>
                    <input type="number" name="sort_order" value="{{ old('sort_order', $locale->sort_order) }}" min="0"
                           class="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-green-500 bg-gray-50 focus:bg-white transition">
                </div>
            </div>
