...
@section('title', __t('admin.edit_location') . ': ' . $location->name)
@section('breadcrumb', __t('admin.panel') . ' / ' . __t('admin.nav_locations') . ' / ' . __t('admin.edit_location'))
        ...
        <form method="POST" action="{{ route('admin.locations.update', $location) }}" class="space-y-4">
            ...
            <div>
                ...
                <select name="type" id="locationType"
                    ...
                    <option value="country" {{ old('type', $location->type) === 'country' ? 'selected' : '' }}>{{ __t('admin.type_country') }}</option>
                    <option value="city"    {{ old('type', $location->type) === 'city'    ? 'selected' : '' }}>{{ __t('admin.type_city') }}</option>
                </select>
            </div>

            {{-- Parent --}}
            <div id="parentField" class="{{ old('type', $location->type) !== 'city' ? 'hidden' : '' }}">
                ...
                <select name="parent_id"
                    ...
                    @foreach($countries as $country)
                        <option value="{{ $country->id }}"
                            {{ old('parent_id', $location->parent_id) == $country->id ? 'selected' : '' }}>
                            {{ $country->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="text" name="name_translations[{{ $locale }}]"
                               value="{{ old('name_translations.' . $locale, $location->name_translations[$locale] ?? ($locale === 'en' ? $location->name : '')) }}"
                               {{ $locale === 'az' ? 'required' : '' }}
            ...
            <div>
                ...
                <input type="text" name="code" value="{{ old('code', $location->code) }}" maxlength="10"
