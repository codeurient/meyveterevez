...
@section('title', __t('admin.add_phone_code'))
@section('breadcrumb', __t('admin.panel') . ' / ' . __t('admin.nav_phone_codes') . ' / ' . __t('admin.add_phone_code'))

@section('content')
<div class="max-w-lg">
    <div class="bg-white rounded-xl border border-gray-100 p-6">
        <form method="POST" action="{{ route('admin.phone-codes.store') }}" class="space-y-4">
            @csrf
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code') }}" required maxlength="10"
                           class="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-green-500 bg-gray-50 focus:bg-white transition font-mono {{ $errors->has('code') ? 'border-red-300' : '' }}"
                           placeholder="AZ">
                    <p class="text-xs text-gray-400 mt-1">{{ __t('admin.code_hint') }}</p>
                </div>

                {{-- Phone code --}}
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">{{ __t('admin.col_phone_code') }}</label>
                    <input type="text" name="phone_code" value="{{ old('phone_code') }}" required maxlength="10"
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="text" name="name_translations[{{ $locale }}]"
                               value="{{ old('name_translations.' . $locale) }}" maxlength="100"
                               {{ $locale === 'en' ? 'required' : '' }}
            ...
            <div>
                ...
                <input type="text" name="native_name" value="{{ old('native_name') }}" required maxlength="100"
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <input type="text" name="trunk_prefix" value="{{ old('trunk_prefix') }}" maxlength="5"
                ...
                <div>
                    ...
                    <input type="text" name="idd_prefix" value="{{ old('idd_prefix') }}" maxlength="10"
                           class="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-green-500 bg-gray-50 focus:bg-white transition font-mono"
                           placeholder="00">
                </div>
            </div>
