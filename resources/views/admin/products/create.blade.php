...
@section('title', __t('admin.add_product'))
@section('breadcrumb', __t('admin.panel') . ' / ' . __t('admin.nav_products') . ' / ' . __t('admin.add_product'))
        ...
        <form method="POST" action="{{ route('admin.products.store') }}" class="space-y-4" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <select name="category_id" required
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-green-500 bg-gray-50 focus:bg-white transition {{ $errors->has('category_id') ? 'border-red-300' : '' }}">
                ...
                <div>
                    ...
                    <select name="store_profile_id" required
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-green-500 bg-gray-50 focus:bg-white transition {{ $errors->has('store_profile_id') ? 'border-red-300' : '' }}">
                        <option value="">— {{ __t('admin.select_store') }} —</option>
                        @foreach($stores as $store)
                            <option value="{{ $store->id }}" {{ old('store_profile_id') == $store->id ? 'selected' : '' }}>
                                {{ $store->name }}
                            </option>
                        @endforeach
                    </select>
                    @error('store_profile_id') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-3">
                ...
                <div>
                    ...
                    <input type="number" name="discount_percent" value="{{ old('discount_percent', 0) }}" min="0" max="100"
                           class="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-green-500 bg-gray-50 focus:bg-white transition"
                           placeholder="0">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <select name="unit"
                        ...
                        <option value="piece" {{ old('unit') === 'piece' ? 'selected' : '' }}>{{ __t('label.piece') }}</option>
                        <option value="bunch" {{ old('unit') === 'bunch' ? 'selected' : '' }}>{{ __t('label.bunch') }}</option>
            ...
            <div class="flex flex-wrap items-center gap-4">
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="{{ $field }}" id="{{ $field }}" value="1"
                           {{ old($field, $field === 'in_stock' ? '1' : '') ? 'checked' : '' }}
